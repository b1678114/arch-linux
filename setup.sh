...
sed -i "s|^GRUB_CMDLINE_LINUX=.*|GRUB_CMDLINE_LINUX=\"rd.luks.name=$(blkid -s UUID -o value /dev/sdb2)=system nmi_watchdog=0 rw quiet splash\"|g" /etc/default/grub
...
sed -i "s|=system|& rd.luks.options=$(blkid -s UUID -o value /dev/sdb2)=tpm2-device=auto|" /etc/default/grub
grub-mkconfig -o /boot/grub/grub.cfg
